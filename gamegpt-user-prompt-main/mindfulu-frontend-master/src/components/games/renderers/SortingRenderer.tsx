...
export const SortingRenderer: React.FC<SortingRendererProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="space-y-6">
          ...
          <CardContent className="space-y-3">
            {getUnplacedItems().length === 0 ? (
              ...
              getUnplacedItems().map(item => (
                ...
                >
                  <p className="font-medium break-words overflow-wrap-anywhere">{item.content}</p>
                </div>
              ))
            )}
          </CardContent>
        ...
        {/* Categories - Below in Columns */}
        <div className="grid grid-cols-[repeat(auto-fill,minmax(400px,1fr))] gap-4">
          {content.categories.map(category => (
            <Card key={category.id} className="border-2 flex flex-col">
              <CardHeader className="flex-shrink-0">
                <CardTitle className="text-lg break-words overflow-wrap-anywhere">{category.name}</CardTitle>
                {category.description && (
                  <p className="text-sm text-muted-foreground break-words overflow-wrap-anywhere">{category.description}</p>
                )}
              </CardHeader>
                  ...
                  {getItemsInCategory(category.id).map(item => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-medium break-words overflow-wrap-anywhere flex-1">{item.content}</p>
  ...
  key={item.id}
  variant="outline"
  className="w-full justify-start break-words whitespace-normal text-left"
      ...
      {showResults && (
        <Card className={`border-2 ${checkCorrectness() === content.items.length ? 'border-green-500 bg-green-50' : 'border-orange-500 bg-orange-50'}`}>
          <CardContent className="p-6 text-center">
            ...
            <h3 className={`text-2xl font-bold mb-2 break-words overflow-wrap-anywhere ${checkCorrectness() === content.items.length ? 'text-green-800' : 'text-orange-800'}`}>
              {checkCorrectness() === content.items.length ? 'Perfect Sorting!' : 'Good Effort!'}
            </h3>
            <p className={`mb-4 break-words overflow-wrap-anywhere ${checkCorrectness() === content.items.length ? 'text-green-700' : 'text-orange-700'}`}>
