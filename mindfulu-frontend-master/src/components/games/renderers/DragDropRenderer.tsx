...
export const DragDropRenderer: React.FC<DragDropRendererProps> = ({
  ...
  const handleDragStart = (e: React.DragEvent, itemId: string) => {
    if (answersSubmitted) return; // Prevent dragging after submission
  ...
  const handleDrop = (e: React.DragEvent, zoneId: string) => {
    ...
    if (!draggedItem || answersSubmitted) return;
  ...
  const handleSubmit = () => {
    ...
    // Calculate final score when submitting
    const correctCount = Object.values(itemStates).filter(state => state === 'correct').length;
    const finalScore = Math.floor((correctCount / content.items.length) * gameSchema.scoring.maxScore);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <Card className="border-2">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              <Badge variant="outline" className="text-sm">
                ...
                {answersSubmitted ? correctCount : '--'}/{content.items.length} Correct
              </Badge>
              <span className="text-sm text-muted-foreground">
                Score: {answersSubmitted ? gameState.score : '--'}/{gameSchema.scoring.maxScore}
              </span>
            </div>
          ...
          <Progress value={answersSubmitted ? progress : (Object.values(droppedItems).flat().length / content.items.length) * 100} className="h-3" />
        </CardContent>
      </Card>
      ...
      <Card className="border-2">
        <CardHeader>
          ...
          <p className="text-muted-foreground break-words overflow-wrap-anywhere">{content.instructions}</p>
        </CardHeader>
        <CardContent>
            ...
            <CardContent>
              <div className="space-y-3">
                {getAvailableItems().map((item) => (
                  <div
                    key={item.id}
                    draggable={!answersSubmitted}
                    onDragStart={(e) => handleDragStart(e, item.id)}
                    className={`p-3 rounded-lg border-2 transition-all select-none hover:shadow-md ${
                      answersSubmitted
                        ...
                        : 'border-border bg-card hover:border-primary/50 cursor-move'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="select-none font-medium break-words overflow-wrap-anywhere">{item.content}</span>
                      {itemStates[item.id] === 'correct' && (
        ...
        <div className="lg:col-span-2">
          <div className="grid gap-4">
            {content.dropZones.map((zone) => (
              <Card
                key={zone.id}
                className={`border-2 border-dashed transition-all ${answersSubmitted ? 'cursor-default' : ''}`}
                onDragOver={answersSubmitted ? undefined : handleDragOver}
                onDrop={answersSubmitted ? undefined : (e) => handleDrop(e, zone.id)}
              >
                <CardHeader>
                ...
                <CardContent>
                  <div className="min-h-[120px] space-y-2">
                          ...
                          className={`p-3 rounded-lg border-2 transition-all select-none ${
                            answersSubmitted
                              ? itemStates[itemId] === 'correct'
                                ...
                                ? 'border-red-500 bg-red-50 text-red-800 cursor-default'
                                : 'border-border bg-card cursor-default'
                              : 'border-border bg-card cursor-move hover:shadow-md'
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <span className="select-none font-medium break-words overflow-wrap-anywhere">{item.content}</span>
                    ...
                    {getItemsInZone(zone.id).length === 0 && (
                      <div className="flex items-center justify-center h-[100px] text-muted-foreground border-2 border-dashed border-muted rounded-lg select-none">
                        Drop items here
                      </div>
                    )}
                  </div>
        ...
        <Card className="border-2 border-red-500 bg-red-50">
          <CardContent className="p-4">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="font-medium text-red-800 mb-1 break-words overflow-wrap-anywhere">Not quite right!</p>
                <p className="text-sm text-red-700 break-words overflow-wrap-anywhere">{showExplanation.explanation}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {gameFinished && (
        <Card className="border-2 border-green-500 bg-green-50">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-xl font-bold text-green-800 mb-2 break-words overflow-wrap-anywhere">
              🎉 Excellent Work!
            </h3>
            <p className="text-green-700 break-words overflow-wrap-anywhere">
